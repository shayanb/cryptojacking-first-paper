...
The term cryptojacking (also known as coinjacking and drive-by mining) was coined to refer to the stealth use of user's resources (e.g. CPU) in the background to mine cryptocurrencies. This paper is focused on in-browser mining, a subset of cryptojacking, in which instead of an application or a malware, the process of mining, commonly written in Javascript, happens within the user's browser by visiting a website. The resources consumption could be really high to noticeably effect users' computer performance and even electricity bill. Users' consent is where makes cryptojacking different than regular mining software. The use of malware to directly install mining software or indirectly enable botnet access 
